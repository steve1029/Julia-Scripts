...
import time, os, datetime, sys, gc
...
class Space(object):
	...
	def Apply_PML(self,PML):
		...
		if self.rank == 0:
			...
			# Delete arrays that will no longer be used in rank 0.

			print(sys.getrefcount(self.space_eps_on))

			del self.space_eps_on
			del self.space_eps_off
			del self.space_mu_on
			del self.space_mu_off

			del self.Esigma_onx
			del self.Esigma_ony
			del self.Esigma_onz

			del self.Esigma_offx
			del self.Esigma_offy
			del self.Esigma_offz

			del self.Hsigma_onx
			del self.Hsigma_ony
			del self.Hsigma_onz

			del self.Hsigma_offx
			del self.Hsigma_offy
			del self.Hsigma_offz

			del self.CDx1, self.CDy1, self.CDz1
			del self.CDx2, self.CDy2, self.CDz2

			del self.CEx1, self.CEy1, self.CEz1
			del self.CEx2, self.CEy2, self.CEz2
			del self.CEx3, self.CEy3, self.CEz3

			del self.CBx1, self.CBy1, self.CBz1
			del self.CBx2, self.CBy2, self.CBz2

			del self.CHx1, self.CHy1, self.CHz1
			del self.CHx2, self.CHy2, self.CHz2
			del self.CHx3, self.CHy3, self.CHz3

			del PML.CDx1, PML.CDy1, PML.CDz1
			del PML.CDx2, PML.CDy2, PML.CDz2

			del PML.CEx1, PML.CEy1, PML.CEz1
			del PML.CEx2, PML.CEy2, PML.CEz2
			del PML.CEx3, PML.CEy3, PML.CEz3

			del PML.CBx1, PML.CBy1, PML.CBz1
			del PML.CBx2, PML.CBy2, PML.CBz2

			del PML.CHx1, PML.CHy1, PML.CHz1
			del PML.CHx2, PML.CHy2, PML.CHz2
			del PML.CHx3, PML.CHy3, PML.CHz3

			del PML

			del send_CDx1, send_CDy1, send_CDz1
			del send_CDx2, send_CDy2, send_CDz2

			del send_CEx1, send_CEy1, send_CEz1
			del send_CEx2, send_CEy2, send_CEz2
			del send_CEx3, send_CEy3, send_CEz3

			del send_CBx1, send_CBy1, send_CBz1
			del send_CBx2, send_CBy2, send_CBz2

			del send_CHx1, send_CHy1, send_CHz1
			del send_CHx2, send_CHy2, send_CHz2
			del send_CHx3, send_CHy3, send_CHz3
