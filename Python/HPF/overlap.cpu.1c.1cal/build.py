...
#			print("rank: {}, tstep : {}, Hy updater : {}" .format(self.rank, tstep, mp.current_process()))
		...
		if self.rank == 1:
			...
			fHHEcstr = 0
			fHHEcendpstr = self.HHEgrid_per_node[2] - 1
			fHHEpend = self.HHEgrid_per_node[2]

			fEEHcstr = 1
			fEEHcendpstr = self.EEHgrid_per_node[2] - 2
		...
		# Indexing child part and parent part of the middle slave nodes.
			...
			mHHEpstr = 1
			mHHEpendcstr = 2
			mHHEcendpstr = self.HHEgrid_per_node[2] - 1
			mHHEpend = self.HHEgrid_per_node[2]

			mEEHpstr = 0
			mEEHpendcstr = 1
			mEEHcendpstr = self.EEHgrid_per_node[2] - 2
		...
		if self.rank == 1:
			...
			parent_Hxupdater(fHHEcendpstr,fHHEpend)
			parent_Hyupdater(fHHEcendpstr,fHHEpend)
			parent_Hzupdater(fEEHcendpstr,fEEHpend)

			# Isend 2D array from the next rank.
		...
		if self.rank > 1 and self.rank < (self.size-1):
			...
			parent_Hxupdater(mHHEcendpstr,mHHEpend)
			parent_Hyupdater(mHHEcendpstr,mHHEpend)
			parent_Hzupdater(mEEHcendpstr,mEEHpend)

			# Isend 2D array to the next rank.
			send_Hx_last = self.comm.Isend(self.Hx[:,:,-1].copy(), dest=(self.rank+1), tag=(tstep*100+1))
			send_Hy_last = self.comm.Isend(self.Hy[:,:,-1].copy(), dest=(self.rank+1), tag=(tstep*100+2))

			self.Hx[:,:,mHHEpstr-1] = QHxto_p.get()
			self.Hy[:,:,mHHEpstr-1] = QHyto_p.get()
