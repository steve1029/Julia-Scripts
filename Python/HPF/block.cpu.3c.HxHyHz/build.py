...
#					self.Hz[:,:,k] = CHz1_k * self.Hz[:,:,k] + CHz2_k * self.Bz[:,:,k] + CHz3_k * previous_k
		...
		if self.rank > 0:
			...
			if platform.system() == 'Linux':
				...
				updateHx = mp_ctx.Process(target=Hxupdater, args=(QEy, QBx, QHx))
				updateHy = mp_ctx.Process(target=Hyupdater, args=(QEx, QBy, QHy))
				updateHz = mp_ctx.Process(target=Hzupdater, args=(QBz, QHz))
				updateHx.start()
				updateHy.start()
				updateHz.start()
		...
		if self.rank > 0 and self.rank < (self.size-1): # rank 1,2,...,n-2
			...
			QEx.put(recv_Ex_last)
			QEy.put(recv_Ey_last)
	...
	def updateE(self, tstep) :
		...
		def Exupdater(QHy, QDx, QEx):
			...
			if self.rank == 1:
				...
				for k in range(zz-1):
					...
					if k == (zz-2):
						recv_Hy_first  = QHy.get()
						self.Hy[:,:,0] = recv_Hy_first

					previous = self.Dx[:,:,-k-2].copy()
					...
					diffzHy_k = (self.Hy[:,:,-k-1] - self.Hy[:,:,-k-2]) / self.dz 
					diffyHz_k = ift(self.iky * ft(self.Hz[:,:,-k-2].copy(), axes=(1,)), axes=(1,))

					self.Dx[:,:,-k-2] = CDx1 * self.Dx[:,:,-k-2] + CDx2 * (diffyHz_k - diffzHy_k)
					self.Ex[:,:,-k-2] = CEx1 * self.Ex[:,:,-k-2] + CEx2 * self.Dx[:,:,-k-2] + CEx3 * previous
				...
				for k in range(zz):
					...
					diffzHy_k = (self.Hy[:,:,-k-1] - self.Hy[:,:,-k-2]) / self.dz 
					diffyHz_k = ift(self.iky * ft(self.Hz[:,:,-k-1].copy(), axes=(1,)), axes=(1,))

					self.Dx[:,:,-k-1] = CDx1 * self.Dx[:,:,-k-1] + CDx2 * (diffyHz_k - diffzHy_k)
					self.Ex[:,:,-k-1] = CEx1 * self.Ex[:,:,-k-1] + CEx2 * self.Dx[:,:,-k-1] + CEx3 * previous
		...
		def Eyupdater(QHx, QDy, QEy):
			...
			if self.rank == 1:
				...
				for k in range(zz-1):
					...
					if k == (zz-2):
						recv_Hx_first  = QHx.get()
						self.Hx[:,:,0] = recv_Hx_first

					previous = self.Dy[:,:,-k-2].copy()
					...
					diffzHx_k = (self.Hx[:,:,-k-1] - self.Hx[:,:,-k-2]) / self.dz 
					diffxHz_k = ift(self.ikx * ft(self.Hz[:,:,-k-2], axes=(0,)), axes=(0,))

					self.Dy[:,:,-k-2] = CDy1 * self.Dy[:,:,-k-2] + CDy2 * (diffzHx_k - diffxHz_k)
					self.Ey[:,:,-k-2] = CEy1 * self.Ey[:,:,-k-2] + CEy2 * self.Dy[:,:,-k-2] + CEy3 * previous
				...
				for k in range(zz):
					...
					diffzHx_k = (self.Hx[:,:,-k-1] - self.Hx[:,:,-k-2]) / self.dz 
					diffxHz_k = ift(self.ikx * ft(self.Hz[:,:,-k-1], axes=(0,)), axes=(0,))

					self.Dy[:,:,-k-1] = CDy1 * self.Dy[:,:,-k-1] + CDy2 * (diffzHx_k - diffxHz_k)
					self.Ey[:,:,-k-1] = CEy1 * self.Ey[:,:,-k-1] + CEy2 * self.Dy[:,:,-k-1] + CEy3 * previous
		...
		if self.rank > 0:
			...
			if platform.system() == 'Linux':
				...
				# Queue for Hx updater.
				QHy = mp_ctx.Queue()
				QDx = mp_ctx.Queue()
				QEx = mp_ctx.Queue()

				# Queue for Hy updater.
				QHx = mp_ctx.Queue()
				QDy = mp_ctx.Queue()
				QEy = mp_ctx.Queue()

				# Queue for Hz updater.
				QDz = mp_ctx.Queue()
				QEz = mp_ctx.Queue()
		...
		if self.rank > 1 and self.rank < self.size: # rank 2,3,...,n-1
			...
			QHx.put(recv_Hx_first)
			QHy.put(recv_Hy_first)
