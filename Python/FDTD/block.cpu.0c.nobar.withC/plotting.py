...
class Graphtool(object):
	...
	def plot2D3D(self, Fields, what, step, xidx=None, yidx=None, zidx=None,**kwargs):
		...
		if Fields.rank == 0 : 
			...
			if what == 'Ex' or what == 'Ey':

				for part in range(1,len(self.gathered_fields)):

					except_last = slice(0,-1)
					if part == (len(self.gathered_fields)-1) : except_last = slice(0,None) 
					portion = Fields.OWN_grid_zslices[part]
					integrated_field[:,:,portion] = self.gathered_fields[part][:,:,except_last]

			elif what == 'Hx' or what == 'Hy':

				for part in range(1,len(self.gathered_fields)):

					except_first = slice(1,None)
					if part == 1: except_first = slice(0,None) 
					portion = Fields.OWN_grid_zslices[part]
					integrated_field[:,:,portion] = self.gathered_fields[part][:,:,except_first]

			elif what == 'Ez' or what == 'Hz':

				for part in range(1,len(self.gathered_fields)):

					entire = slice(None,None)
					portion = Fields.OWN_grid_zslices[part]
					integrated_field[:,:,portion] = self.gathered_fields[part][:,:,entire]

			else:
				print("Wrong field! Simulation aborted.")
				sys.exit()
